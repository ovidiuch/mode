...
Path.prototype = {
  ...
  view: function(output) {
    ...
    // Attempt to load view when a path for it was found
    if (path && typeof(output = mode.view.open(path, this.args)) != 'string') {
      mode.throw(404, 'Invalid view ' + path);
    }
    // Attempt to load view layout if specified
    if (typeof(output) == 'string' && this.args.layout) {
      this.args.content = output;
      path = '/layout/' + this.args.layout;
      if (typeof(output = mode.view.open(path, this.args)) != 'string') {
