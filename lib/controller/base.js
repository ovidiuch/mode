...
module.exports = require('../class/base.js').extend(function(parent) {
   ...
   *
   * Even though it is optional, it should always be defined
   * as a valid object, in order to eliminate the need for
   * excessive safe-checks when working around it.
   *
   * @type {object}
   */
  this.data = {};
   ...
   * Custom path setter.
   *
   * Besides a back-reference to the parent path instance,
   * which is useful when loading a new one, this setter also
   * binds the path's domain to the controller instance and
   * creates useful shorthand references to relevant properties.
   *
  ...
  this.__defineSetter__('path', function(path) {
    this.path = path;
    // Bind path's domain to controller instance; essential for
    // gracefully catching exceptions
  ...
  this.load = function(query, args, callback) {
    ...
    if (path.keys(query).length > 1) {
      ...
      // Path on path properties to new one, if a current one
      // exists
      if (this.path) {
