...
module.exports = require('../class/base.js').extend(function(parent) {
   ...
   * @param {string} query
   * @param {object} args
   * @throws {Exception} When server request is missing
   * @see path.redirect
   */
   ...
   * @throws {Exception} If requested method is invalid
   * @see path.load
   * @see #call
   */
  this.load = function(query, args, callback) {
    ...
    if (path.keys(query).length > 2) {
      // Use current callback if a new one isn't specified.
      // Path will be rendered without a callback if there's no
      // current one.
      path.render(query, args, callback || this.callback);
      return;
    }
   ...
   * @throws {Exception} If a view path can't be loaded
   */
  this.render = function(action, response) {
    // Build default, implicit view path
    var path = this.static.path + '/' + action;
    ...
    if (response != null) {
      ...
      // Overwrite the view path with the response itself, if
      // it's an actual view path, otherwise return it directly
    ...
    var output = view.render(path, this.args);
