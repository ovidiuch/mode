...
module.exports = require('../class/base.js').extend(function(parent) {
   ...
   * @param {function} callback
   * @throws {Error} If view paths are invalid
   */
  this.on('render-default', function(action, response, callback) {
    ...
    var actionPath = view.getPath(
      action, '/action' + this.static.path, this.format
    );
    // Init empty output
    var output;
    ...
    if (output == null) {
      // Throw if a valid view path could not be established
      if (!actionPath) {
        error.throw(404, 'No view path for action ' + action);
      }
      // Render view with controller vars
      output = view.render(actionPath, this.vars);
    }
    // Attempt to render view layout, if specified
    if (this.layout) {
      var layoutPath = view.getPath(this.layout, '/layout', this.format);
      // Ignore a missing layout path for specific formats, but
      // throw error on default one
      if (layoutPath) {
        // Render view layout with controller vars and initial
        // view output as {{content}}
        this.vars.content = output;
        output = view.render(layoutPath, this.vars);
      } else if (!this.format) {
        error.throw(404, 'Invalid layout name ' + this.layout);
      }
    }
    callback(output);
  });
