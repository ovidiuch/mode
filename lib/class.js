 ...
 * but it also tells you the type of class you are loading.
 *
 * Example:
 *
 ...
 * @see mode.file.index
 */
exports.index = function(path, type) {
  if (type) {
    path += '/' + type;
  }
  mode.file.index(path, true, function(handle, module) {
    if (type) {
      handle = type + '/' + handle;
    }
    console.log('Loading class: ' + handle);
  });
};
/**
 * Fetch class by name handle.
 *
 * Example:
 *
 *     mode.class.get('controller/admin/foo-bar');
 *     mode.class.get('admin/foo-bar', mode.controller);
 *     mode.class.get('foo-bar', mode.controller.admin);
 *     // will all look for mode.controller.admin.FooBar
 * 
 * @param {string} handle Class handle
 * @param {string} scope Parent scope
 * @return {object} Found class object
 * @see #name
 */
exports.get = function(handle, scope) {
  scope = scope || mode;
 ...
 * Extend a class.
 *
 * The biggest difference between most of the other existing
 * class models and how _mode_ works is that the class is not
 * defined by an object with a set of properties and methods,
 * but by a function with instructions.
 *
 ...
 * the new class, but the Class object is also referenced by
 * `this.static`. This obviously can be used to define static
 ...
 * an instance of the parent class as the first parameter.
 *
 * The parent instance received will also have the `.static`
...
Base.extend = function(definition, abstract) {
  ...
  Class.prototype = new definition(new this());
  return Class;
};
/**
...
/**
 * Clone class property.
 *
 * For simplicity reasons, only plain objects are being cloned.
 * 
 ...
 * Example:
 *
 *     name('foo-bar'); // returns "FooBar"
 *
 * TODO: Move to an util package, when needed in other places.
 * TODO: Add this to controller actions.
 * 
 * @param {string} slug
