 ...
 * @see mode.file.index
 */
exports.index = function(path, namespace) {
  if (namespace) {
    path += '/' + namespace;
  }
  mode.file.index(path, true, function(path, module) {
    if (namespace) {
      path = '/' + namespace + path;
    }
    console.log('Loading class: ' + path);
    // Set path prefix to /class if it has none
    if (mode.path.keys(path).length < 2) {
      path = '/class' + path;
    }
