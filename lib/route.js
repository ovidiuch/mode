  ...
  for (var i in list) {
    ...
    for (var j in list[i].args) {
      args[j] = list[i].args[j].replace(/\$([0-9]+)/, function() {
        // Make sure to return an empty string for null values,
        // instead of a stringified evaluation, like "undefined"
    ...
    if (list[i].args.redirect) {
      return this.match(mustache.parse(list[i].args.redirect, args), method);
    }
    return args;
  }
  return false;
