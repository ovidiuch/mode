 ...
 *     mode.route.add('/:controller/@action');
 *     // Same route, in RegExp
 *     mode.route.add(/^\/([a-z0-9-/]+)\/([a-z0-9-]+)$/, {
 ...
 *     });
 *
 * String patterns can only match queries entirely, from start
 * to end. RegExp patterns, however, can also be used to match
 * specific query parts only.
 *
 * RegExp example:
 *
 *     // Route for mapping anything that starts with '/user-'
 *     mode.route.add(/^\/user-/, {controller: 'user', action: 'index'});
 * 
 ...
 * patterns) upon adding, with their values replaced by
 * corresponding query matches.
 *
 * Example:
 *
 *     // Considering the route.add examples
 *     mode.route.match('/post');
 *     // returns {controller: 'post', action: 'index'}
 *     mode.route.match('/post/create', 'POST');
 *     // returns {controller: 'post', action: 'create'}
 *     mode.route.match('/post/delete');
...
exports.match = function(query, method) {
  ...
  for (var i in list) {
    ...
    if (list[i].args.redirect) {
      return this.match(mode.mustache.parse(list[i].args.redirect, args));
    }
