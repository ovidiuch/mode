 ...
 *
 * The _prototype_ parameter becomes the prototype of ScopeArguments.
 *
 * Any set of arguments that a Scope instance receives will
 * inherit the helper object the module was initialized with.
 *
 * Incidentally, both a helper can be a variable and a scope argument
 * a function. The mustache parser will interpret them accordingly.
 *
 * Example:
 *
 ...
 * Example:
 *
 *     mode.mustache.parse('My name is {{name}}', {name: 'Earl'});
 * 
 ...
 *
 * Every scope has its own set of arguments, which can come from
 * four different places and get overridden as follows:
 *
 *   - helpers
 *   - parent scopes
 ...
 *
 * Besides the _args_ property, a Scope also has a _this_ property, 
 * which is the subject of the current scope. It comes as the
 * second parameter, and normally, only present in child scopes.
 *
 * If the subject is an object, all its properties will be poured
 * into the scope argument pool.
 * 
 * @constructor
 * @param {object} args
...
Scope.prototype = {
   ...
   *
   * If the section subject is an Array, the section template body
   * will loop and apply for each of its elements. Otherwise, it
   * will just apply once for contents of the subject; and only if
   * it's not null, or viceversa, with the _inverted_ modifier.
   * 
   * Examples:
   *
   *     <!--normal section-->
   ...
   * 
   * TODO: Make mustache module mode-independent, and run
   * partials through a used-defined callback.
   *
   * Example:
   *
  ...
  params: function(list) {
    var arguments = [], scope = this;
    list.forEach(function(param) {
      arguments.push(scope.argument(param));
    });
    return arguments;
  },
  /**
