...
var Mode = require('./class/base.js').extend(function() {
  ...
  this.init = function(settings) {
    ...
    this.class.index(__dirname + '/class', this);
    this.emit('init');
  };
  /**
  ...
  this.setup = function(basePath) {
    ...
    for (var i in types) {
      this.class.index(p.app, this, types[i]);
    }
  ...
  this.on('request', function(query, args, conn) {
    ...
    var d = conn.domain = domain.create();
    d.on('error', this.bind(function(err) {
      error.handle(err, conn);
    }));
    d.run(this.bind(function() {
      path.load(query, args, conn);
    }));
  });
});
