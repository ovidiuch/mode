...
var Mode = require('./class/base.js').extend(function() {
  ...
  this.init = function(settings) {
    ...
    var types = ['class', 'collection', 'model', 'controller'];
  ...
  this.setup = function(basePath) {
    ...
    var types = p.types || ['model', 'collection', 'controller'];
    types.forEach(this.bind(function(type) {
  ...
  this.start = function(settings, callback) {
    ...
    this.server = server.start(this.settings.server, this.bind(function(conn) {
      var parts = url.parse(conn.req.url, true);
      var args = path.args(parts.query);
      this.emit('request', parts.pathname, args, conn);
    }));
    this.emit('start');
  };
  /**
  ...
  this.on('request', function(query, args, conn) {
    ...
    // Init cookie group for current response and tie it to the
    // connection instance
    conn.cookie = cookie.init(conn.req, conn.res);
