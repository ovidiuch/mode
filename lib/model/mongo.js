...
module.exports = require('./base.js').extend(function(parent) {
  ...
  this.static.list = function(args, callback) {
    ...
    this.collection.find(query, this.bind(function(error, cursor) {
      if (error) {
        this.throw(500, error);
      }
      var items = [];
  ...
  this.static.item = function(args, callback) {
    // Process args by running them through a fresh instance
    var query = new this(args).data;
    this.collection.findOne(query, this.bind(function(error, item) {
      if (error) {
        this.throw(500, error);
      }
  ...
  this.static.delete = function(args, callback) {
    // Process args by running them through a fresh instance
    var query = new this(args).data;
    this.collection.remove(query, this.bind(function(error) {
      if (error) {
        this.throw(500, error);
      }
      callback();
  ...
  this.save = function(callback) {
    this.static.collection.save(this.data, {safe: true}, this.bind(function(error, item) {
      if (error) {
