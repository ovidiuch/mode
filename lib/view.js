 ...
 * Get view path.
 *
 * Build and validate view path. A null value will be returned
 * if the resulted view path does not exist.
 *
 * The namespace and format are optional.
 *
 * Example:
 *
 *     view.getPath('index', 'json', '/action/user');
 *     // would look for /action/user/json/index.mustache
 *
 * @param {string} name
 * @param {string} format
 * @param {string} namespace
 * @return {string} Valid view path
 */
exports.getPath = function(name, namespace, format) {
  var path = (namespace || '') + '/';
  if (format) {
    path += format + '/';
  }
  path += name;
  return exports.exists(path) ? path : null;
}
/**
